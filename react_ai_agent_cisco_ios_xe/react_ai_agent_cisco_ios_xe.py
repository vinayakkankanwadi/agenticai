...
def run_show_command(command: str):
    try:
        ...
        parsed_output = device.execute("uci get network.lan.ipaddr")
...
def check_command_support(command: str) -> dict:
    ...
    #close_matches='uci show network.lan.ipaddr'
    #print(command_list)
    #print(close_matches)
...
def check_supported_command_tool(command: str) -> dict:
    ...
    if result.get('status') == 'supported':
        ...
        closest_command = 'uci show network.lan.ipaddr'
        return {
            "status": "supported",
...
Action Input: "uci show network.lan"  
